...
CompletedData<-filter(PracticeData, Completed==1, AttendedFollowUp==1,
                      PSE1!=999, PSE1!=2222, PSE3!=999, PSE3!=2222)
...
CompletedData$PSEchange <- CompletedData$PSE3-CompletedData$PSE1
...
sd_data <- data.frame(
  term = c("PSEchange", "RMQ1", "CPAQtot1", "Intensity1", "PCS1", "sitstan1"),
  sd = c(
...
glm_models_mice <- with(ImputedData,
                        glm(PSEchange ~ RMQ1 + CPAQtot1 + Intensity1 + PCS1 +
                              sitstan1 + NoPrep +
                              PrimaryDiagnosisGroup + Litigation))
...
#   all_predictors <- c("RMQ1", "CPAQtot1", "Intensity1",
#                       "PCS1", "sitstan1",
#                       "NoPrep", "PrimaryDiagnosisGroup", "Litigation")
#   
...
# continuous_predictors <- c("RMQ1", "CPAQtot1", "Intensity1",
#                            "PCS1", "sitstan1")
# categorical_predictors <- c("NoPrep", "PrimaryDiagnosisGroup", "Litigation")
# 
...
for (i in 1:m) {
  ...
  # Fit model
  model_i <- glm(PSEchange ~ RMQ1 + CPAQtot1 + Intensity1 + PCS1 +
                   sitstan1 + NoPrep + PrimaryDiagnosisGroup + Litigation,
                 data = data_i)
...
cat("\n========================================\n")
cat("SHRUNKEN COEFFICIENTS (Calibration Slope = 0.644)\n")
cat("========================================\n")
...
cat("Shrinkage Factor (Calibration Slope): 0.644 \n")
cat("========================================\n")
