## Primary model (T1-T0 model for PSE)
...
# subset dataset with all predictor variables of potential use (no PSE1)
...
for (i in 1:20) {
  ...
  mfp2_model <- mfp2(
    PSEchange ~ fp(age) + fp(duration) + fp(Intensity1) + fp(Distress1) + 
...
sd_data <- data.frame(
  term = c("PSEchange", "RMQ1", "CPAQtot1", "PCS1", "Intensity1", "Distress1", "Satisfaction1", "sitstan1"),
  sd = c(
...
for (i in 1:m) {
  ...
  # Fit model
  model_i <- glm(PSEchange ~ RMQ1 + CPAQtot1 + PCS1 + Intensity1 +
                   Distress1 + Satisfaction1 + sitstan1 + Online + NoPrep +
                   PrimaryDiagnosisGroup + Employment,
                 data = data_i)
...
cal_plot_overall <- ggplot(all_preds_obs, aes(x = Predicted, y = Observed)) +
  ...
  geom_smooth(method = "loess", color = "blue", se = FALSE, alpha = 0.3) +  # Flexible calibration curve
  ...
  labs(
    ...
    x = "Predicted PSE Change",
    y = "Observed PSE Change"
  ) +
  theme_minimal() +
  theme(
...
decile_plot <- ggplot(cal_data_deciles, aes(x = MeanPred, y = MeanObs)) +
  ...
  labs(
    ...
    x = "Mean Predicted PSE Change",
    y = "Mean Observed PSE Change",
...
dist_plot <- all_preds_obs %>%
  pivot_longer(cols = c(Predicted, Observed), 
               names_to = "Type", 
               values_to = "PSEchange") %>%
  ggplot(aes(x = PSEchange, fill = Type)) +
...
residual_plot <- ggplot(residual_data, aes(x = Predicted, y = Residual)) +
  ...
  geom_smooth(method = "loess", color = "blue", se = FALSE, alpha = 0.3) +
  labs(
    title = "Residual Plot",
...
ggsave("distribution_plot.png", dist_plot, width = 12, height = 6, dpi = 300)
...
# Prepare data for coefficient plots (exclude intercept)
plot_data <- results_table[results_table$Variable != "(Intercept)", ]
...
original_order <- results_table %>%
  dplyr::filter(Variable != "(Intercept)") %>%
  dplyr::pull(Variable)

# Prepare long format data - INCLUDING categorical variables
plot_long <- results_table %>%
  dplyr::filter(Variable != "(Intercept)") %>%
  dplyr::select(Variable, Coefficient, `Standardised Beta`, CI_Lower, CI_Upper) %>%
  ...
  dplyr::mutate(
    ...
    Variable = factor(Variable, levels = rev(original_order)), # rev() because ggplot shows bottom to top
    ...
    CI_Lower_adj = ifelse(Type == "Standardised Beta", NA, CI_Lower),
    CI_Upper_adj = ifelse(Type == "Standardised Beta", NA, CI_Upper)
  )

# Create publication-ready faceted forest plot with original order
p <- ggplot(plot_long, aes(x = Value, y = Variable)) +  # No reordering here
  ...
  # Add error bars (only for unstandardised coefficients)
  geom_errorbarh(aes(xmin = CI_Lower_adj, xmax = CI_Upper_adj), 
                 height = 0, 
                 size = 0.6, 
                 color = "black",
                 na.rm = TRUE) +
  ...
  # Add point estimates
  geom_point(shape = 18,  # Diamond shape
             size = 3.5, 
             color = "black") +
  ...
  facet_wrap(~ factor(Type, levels = c("Unstandardised Coefficients", "Standardised Beta")), 
             scales = "free_x",
             nrow = 1) +  # Put panels side by side
...
ggsave("forest_plot_coefficients.pdf", 
       plot = p,
       width = 10, 
       height = 6, 
       dpi = 300)
...
ggsave("forest_plot_coefficients.png", 
       plot = p,
       width = 10, 
       height = 6, 
       dpi = 600,
...
# Create publication-ready faceted forest plot with color-coded significance
p_colored <- ggplot(plot_long, aes(x = Value, y = Variable, color = significant)) +
  # Add reference line at zero
  geom_vline(xintercept = 0, 
             linetype = "solid", 
             color = "black", 
  ...
  # Add error bars (now for both types)
  geom_errorbarh(aes(xmin = CI_Lower_adj, xmax = CI_Upper_adj), 
                 height = 0, 
                 size = 0.6,
                 na.rm = TRUE) +
  ...
  scale_color_manual(values = c("Significant (p < 0.05)" = "#D55E00",  # Deep orange
...
ggsave("forest_plot_colored_significance.pdf", 
       plot = p_colored,
       width = 10, 
       height = 6.5,
       dpi = 300)
...
ggsave("forest_plot_colored_coefficients.png", 
       plot = p_colored,
       width = 10, 
       height = 6, 
       dpi = 600,
...
## Defining  custom labels (names = existing Variable values, values = new labels)
variable_labels <- c(
  "NoPrep1" = "At least one prep session",
  "Satisfaction1" = "Satisfaction",
  "Intensity1" = "Pain Intensity",
  "RMQ1" = "RMDQ",
  "CPAQtot1" = "CPAQ",
  "Distress1" = "Pain Distress",
  "Online1" = "Face-to-face",
  "PrimaryDiagnosisGroup2" = "Spinal/Failed back surgery",
  "PrimaryDiagnosisGroup3" = "Chronic Regional Pain Syndrome",
  "PrimaryDiagnosisGroup4" = "Location-specific pain",
  "PrimaryDiagnosisGroup5" = "Facial pain",
  "PrimaryDiagnosisGroup6" = "Migrane/headache",
  "PrimaryDiagnosisGroup7" = "Pelvic/Abdominal pain",
  "PrimaryDiagnosisGroup8" = "Neuropathic pain",
  "PrimaryDiagnosisGroup9" = "Arthritic/rheumatological pain",
  "sitstan1" = "One-Minute Sit-to-Stand",
  "PCS1" = "PCS",
  "Employment2" = "Part-time paid employment",
  "Employment3" = "Not working due to pain",
  "Employment4" = "Not working for other reasons",
  "Employment5" = "Not working due to other health condition",
  "Employment6" = "Retired"
)
...
cat("\n========================================\n")
cat("SHRUNKEN COEFFICIENTS (Calibration Slope = 0.885)\n")
cat("========================================\n")
...
cat("Shrinkage Factor (Calibration Slope): 0.885\n")
cat("Shrinkage Applied: 12.3%\n")
cat("========================================\n")
...
plot_long <- results_table_shrunk %>%
  ...
  dplyr::select(Variable, Coefficient, `Standardised Beta`, CI_Lower, CI_Upper, significant) %>%
  tidyr::pivot_longer(cols = c(Coefficient, `Standardised Beta`),
                      names_to = "Type",
                      values_to = "Value") %>%
  dplyr::filter(!is.na(Value)) %>%
...
# Create publication-ready faceted forest plot with color-coded significance
p_colored_shrunk <- ggplot(plot_long, aes(x = Value, y = Variable, color = significant)) +
  # Add reference line at zero
  geom_vline(xintercept = 0, 
             linetype = "solid", 
             color = "black", 
...
print(p_colored_shrunk)

# Save the colored version
ggsave("forest_plot_shrunk_colored_significance.pdf", 
       plot = p_colored_shrunk,
       width = 10, 
       height = 6.5,  # Slightly taller to accommodate legend
       dpi = 300)

# Save as high-resolution PNG (for presentations/web)
ggsave("forest_plot_shrunk_colored_significance.png", 
       plot = p_colored_shrunk,
       width = 10, 
       height = 6, 
       dpi = 600,
...
variable_labels <- c(
  ...
  "Distress1" = "Pain Distress",
  "Online" = "Face-to-face",
  "PrimaryDiagnosisGroup" = "Primary Diagnosis",
  "sitstan1" = "One-Minute Sit-to-Stand",
  "PCS1" = "PCS"
)
