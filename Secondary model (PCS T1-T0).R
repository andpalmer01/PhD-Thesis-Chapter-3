## Secondary model (T1-T0 model for PCS)
...
CompletedData<-filter(PracticeData, Completed==1, PCS1!=999, PCS1!=2222,
                      PCS2!=999, PCS2!=2222, PCS2!=60)
...
CompletedData$PCSchange <- CompletedData$PCS2-CompletedData$PCS1
...
sd_data <- data.frame(
  term = c("PCSchange", "BDI1", "RMQ1", "CPAQtot1", "PSE1", "Distress1", "Performance1", "Satisfaction1", "sitstan1", "Walk5min1"),
  sd = c(
    ...
    sd(SubsetData$Performance1, na.rm = TRUE),
    sd(SubsetData$Satisfaction1, na.rm = TRUE),
    sd(SubsetData$sitstan1, na.rm = TRUE),
    sd(SubsetData$Walk5min1, na.rm = TRUE)
  )
)
...
glm_models_mice <- with(ImputedData,
                        glm(PCSchange ~ BDI1 + RMQ1 + CPAQtot1 + PSE1 +
                              Distress1 + Performance1 + Satisfaction1 +
                              sitstan1 + Walk5min1 + PrimaryDiagnosisGroup +
                              Employment))
...
sd_independent_vars <- c(
  ...
  "Performance1" = sd_data$sd[sd_data$term == "Performance1"],
  "Satisfaction1" = sd_data$sd[sd_data$term == "Satisfaction1"],
  "sitstan1" = sd_data$sd[sd_data$term == "sitstan1"],
  "Walk5min1" = sd_data$sd[sd_data$term == "Walk5min1"]
)
...
#   all_predictors <- c("BDI1", "RMQ1", "CPAQtot1", "PSE1", 
#                       "Distress1", "Performance1", "Satisfaction1", 
#                       "sitstan1", "Walk5min1", "PrimaryDiagnosisGroup",
#                       "Employment")
#   
...
# continuous_predictors <- c("BDI1", "RMQ1", "CPAQtot1", "PSE1", 
#                            "Distress1", "Performance1", "Satisfaction1", 
#                            "sitstan1", "Walk5min1")
# categorical_predictors <- c("PrimaryDiagnosisGroup", "Employment")
# 
...
for (i in 1:m) {
  ...
  # Fit model
  model_i <- glm(PCSchange ~ BDI1 + RMQ1 + CPAQtot1 + PSE1 +
                   Distress1 + Performance1 + Satisfaction1 +
                   sitstan1 + Walk5min1 + PrimaryDiagnosisGroup +
                   Employment,
                 data = data_i)
...
cat("\n========================================\n")
cat("SHRUNKEN COEFFICIENTS (Calibration Slope = 0.894)\n")
cat("========================================\n")
...
cat("Shrinkage Factor (Calibration Slope): 0.894\n")
cat("========================================\n")
