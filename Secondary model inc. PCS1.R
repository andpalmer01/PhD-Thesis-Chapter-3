## Secondary model including PCS1 as a predictor variable
...
SubsetData <- CompletedData%>%dplyr::select(age,PCS1,duration,Intensity1,Distress1,
...
for (i in 1:20) {
  ...
  mfp2_model <- mfp2(
    PCSchange ~ fp(age) + fp(PCS1) + fp(duration) + fp(Intensity1) + fp(Distress1) + 
...
sd_data <- data.frame(
  term = c("PCSchange", "age", "PCS1", "BDI1", "RMQ1", "CPAQtot1", "PSE1", "Distress1", "Satisfaction1", "Walk5min1"),
  sd = c(
...
glm_models_mice <- with(ImputedData,
                        glm(PCSchange ~ age + PCS1 + BDI1 + RMQ1 + CPAQtot1 + PSE1 +
                              Distress1 + Satisfaction1 +Walk5min1 + PrimaryDiagnosisGroup +
                              Employment))
...
for (i in 1:m) {
  ...
  # Fit model
  model_i <- glm(PCSchange ~ age + PCS1 + BDI1 + RMQ1 + CPAQtot1 + PSE1 +
                   Distress1 + Satisfaction1 + Walk5min1 + PrimaryDiagnosisGroup +
                   Employment,
                 data = data_i)
